...
fun BackdropEffectScope.lens(
    ...
    val key = if (dispersionEnabled) "FunLinkLiquidGlassLensDispersion" else "FunLinkLiquidGlassLens"

    val scaleFactor = downscaleFactor.coerceAtLeast(1).toFloat()
    val scaledSizeW = size.width / scaleFactor
    val scaledSizeH = size.height / scaleFactor
    val scaledPadding = padding / scaleFactor
    val scaledRefractionHeight = refractionHeight / scaleFactor
    val scaledRefractionAmount = refractionAmount / scaleFactor
    val scaledRadii = FloatArray(radii.size) { radii[it] / scaleFactor }
